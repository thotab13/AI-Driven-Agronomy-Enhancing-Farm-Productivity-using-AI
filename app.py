...
from predictor import fertilizer_prediction, yield_prediction

app = Flask('ML project')
...
def prediction_fertilizer():
    if request.method == 'POST':
        ...
        predictions = fertilizer_prediction(temperature, humidity, moisture, soil_type, crop_type, N, K, P) 
        # Return the predictions as JSON response
        print(predictions)
        return jsonify({'prediction':predictions})
    else:
        return render_template('predictions_fertilizer.html')
    


@app.route('/predict/yield', methods=['GET','POST'])
def prediction_yield():
    if request.method=='POST':
        # Get the JSON data from the POST request
        data = request.get_json()

        # Assuming the JSON data contains keys corresponding to feature names
        # Extract features from the JSON data
        Temp = data.get('Temp')
        ...
        # Perform prediction using your model function
        predictions = yield_prediction(Temp, Precipitation, SOM, AWC, Land_Area, VPD)

        # Return the predictions as JSON response
        return jsonify({'predictions': predictions})
    else:
        return render_template('predictions_yield.html')
